...
<#
.SYNOPSIS
Checks if a release represents the highest non-prerelease patch version.

.DESCRIPTION
Determines if a given release is the "latest" production release that users
would get when using v1, v1.0, etc. This is the release that should be
...
.OUTPUTS
Returns $true if this release is the highest non-prerelease patch, $false otherwise.
#>
function Test-IsHighestNonPrereleaseVersion {
    param(
        [Parameter(Mandatory)]
        [RepositoryState]$State,
