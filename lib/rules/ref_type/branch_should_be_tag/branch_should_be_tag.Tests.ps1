...
Describe "branch_should_be_tag" {
    ...
    Context "CreateIssue" {
        ...
        It "uses highest patch SHA for major branch conversion when branch SHA is stale" {
            $state = [RepositoryState]::new()
            $staleBranch = [VersionRef]::new("v6", "refs/heads/v6", "767b16506b540fdce093ac79ebb8441f5f0a0e08", "branch")
            $latestPatch = [VersionRef]::new("v6.0.0", "refs/tags/v6.0.0", "452e2d19b81542ae14b68e36469883b9499fca79", "tag")
            $state.Branches += $staleBranch
            $state.Tags += $latestPatch
            $config = @{ 'floating-versions-use' = "tags" }

            $issue = & $Rule_BranchShouldBeTag.CreateIssue $staleBranch $state $config
