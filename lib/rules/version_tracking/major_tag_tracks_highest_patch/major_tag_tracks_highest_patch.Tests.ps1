...
Describe "major_tag_tracks_highest_patch" {
    ...
    Context "Prerelease Filtering" {
        It "should pass Check when major tag exists but only prerelease patches exist and ignore-preview-releases is true" {
            # Edge case: v2 tag exists, but only v2.0.0-preview exists (no stable patches)
            ...
            $state = [RepositoryState]::new()
            # v2 exists
            $majorTag = [VersionRef]::new("v2", "refs/tags/v2", "preview123", "tag")
            $state.Tags += $majorTag
