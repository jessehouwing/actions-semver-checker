...
Describe "minor_tag_missing" {
    Context "Condition - AppliesWhen floating-versions-use is tags and check-minor-version is enabled" {
        ...
        It "should not return missing minor when same version branch exists" {
            $state = [RepositoryState]::new()
            $state.Tags += [VersionRef]::new("v1.0.0", "refs/tags/v1.0.0", "patch123", "tag")
            $state.Branches += [VersionRef]::new("v1.0", "refs/heads/v1.0", "stale456", "branch")
            $state.IgnoreVersions = @()
