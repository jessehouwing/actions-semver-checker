...
Describe "patch_release_required" {
    ...
    Context "Condition - Finding missing releases" {
        ...
        It "should not require v6.1.0 when v6.1 exists and v6.1.1 has a release" {
            $state = [RepositoryState]::new()
            $state.Tags += [VersionRef]::new("v6.1", "refs/tags/v6.1", "sha611", "tag")
            $state.Tags += [VersionRef]::new("v6.1.1", "refs/tags/v6.1.1", "sha611", "tag")
            ...
            $releaseData = [PSCustomObject]@{
                ...
                draft = $false
                prerelease = $false
                html_url = "https://github.com/repo/releases/tag/v6.1.1"
                target_commitish = "sha611"
                immutable = $true
            }
            $state.Releases = @([ReleaseInfo]::new($releaseData))
            $state.IgnoreVersions = @()
