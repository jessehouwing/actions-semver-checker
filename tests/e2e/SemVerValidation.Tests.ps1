...
Describe "SemVer Checker" {
    ...
    Context "When using branches for versions" {
        ...
        It "Should flag patch branch even when commits differ" {
        ...
        It "Should auto-fix ref type respecting floating-versions-use setting (tags mode)" {
            ...
            git branch -D v1.0-temp 2>&1 | Out-Null
            ...
            $result.Output | Should -Not -Match "git push origin :refs/tags/v1.0"
        }
        ...
        It "Should auto-fix ref type respecting floating-versions-use setting (branches mode)" {
            ...
            $result.Output | Should -Not -Match "git push origin :refs/heads/v1.0"
        }
    }
...
'@ | Out-File -FilePath $hookPath -Encoding UTF8
    ...
    Context "Version Logic - Branch and Tag Conflicts" {
        It "Should flag floating branch when tag exists" {
            Initialize-TestRepo -Path $script:testRepoPath -WithRemote
        ...
        It "Should handle branch/tag conflict with floating-versions-use setting" {
            ...
            # Should suggest converting tag to branch
    ...
    Context "Ref Conversion Actions - Auto-fix and Edge Cases" {
        ...
        It "Should convert stale major branch to tag using latest patch SHA without duplicate create commands" {
            Initialize-TestRepo -Path $script:testRepoPath -WithRemote

            $oldCommit = Get-CommitSha

            "# New release" | Out-File -FilePath "README.md" -Append
            git add README.md
            git commit -m "New patch release" 2>&1 | Out-Null
            $newCommit = Get-CommitSha

            git tag v6.0.0 $newCommit

            git branch v6-local $oldCommit
            git push origin v6-local:v6 2>&1 | Out-Null
            git branch -D v6-local 2>&1 | Out-Null

            $result = Invoke-MainScript -FloatingVersionsUse "tags" -CheckMinorVersion "error"

            $result.Output | Should -Match "Branch 'v6' should be a tag"
            $result.Output | Should -Not -Match "v6 does not exist"
            $result.Output | Should -Match "Minor version tag v6.0 does not exist"

            $result.Output | Should -Match "git push origin $newCommit`:refs/tags/v6[^.]"
            $result.Output | Should -Match "git push origin $newCommit`:refs/tags/v6.0[^.]"
            $result.Output | Should -Match "git push origin :refs/heads/v6"

            $outputLines = $result.Output -split "`r?`n"
            $v6CreateCommands = @($outputLines | Where-Object { $_ -match "^\s*git push origin .*:refs/tags/v6\s*$" } | Select-Object -Unique)
            $v60CreateCommands = @($outputLines | Where-Object { $_ -match "^\s*git push origin .*:refs/tags/v6\.0\s*$" } | Select-Object -Unique)
            $v6CreateCommands.Count | Should -Be 1
            $v60CreateCommands.Count | Should -Be 1

            $result.Output | Should -Not -Match "git push origin $oldCommit`:refs/tags/v6[^.]"
        }

        It "Should convert stale major tag to branch using latest patch SHA without duplicate create commands" {
            Initialize-TestRepo -Path $script:testRepoPath -WithRemote
            ...
            git tag v6.0.0 $newCommit
            git tag v6 $oldCommit

            $result = Invoke-MainScript -FloatingVersionsUse "branches" -CheckMinorVersion "error"

            $result.Output | Should -Match "Tag 'v6' should be a branch"
            $result.Output | Should -Not -Match "Major version branch v6 does not exist"
            $result.Output | Should -Match "Minor version branch v6.0 does not exist"

            $result.Output | Should -Match "git push origin $newCommit`:refs/heads/v6[^.]"
            $result.Output | Should -Match "git push origin $newCommit`:refs/heads/v6.0[^.]"
            $result.Output | Should -Match "git push origin :refs/tags/v6"

            $outputLines = $result.Output -split "`r?`n"
            $v6CreateCommands = @($outputLines | Where-Object { $_ -match "^\s*git push origin .*:refs/heads/v6\s*$" } | Select-Object -Unique)
            $v60CreateCommands = @($outputLines | Where-Object { $_ -match "^\s*git push origin .*:refs/heads/v6\.0\s*$" } | Select-Object -Unique)
            $v6CreateCommands.Count | Should -Be 1
            $v60CreateCommands.Count | Should -Be 1

            $result.Output | Should -Not -Match "git push origin $oldCommit`:refs/heads/v6[^.]"
        }
